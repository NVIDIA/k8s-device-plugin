...
func main() {
	...
	c.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			Name:    spec.FlagDriverRoot,
			Aliases: []string{spec.FlagNvidiaDriverRoot},
			Value:   "/",
			Usage:   "the root path for the NVIDIA driver installation on the host (typical values are '/' or '/run/nvidia/driver')",
			EnvVars: []string{"NVIDIA_DRIVER_ROOT"},
		},
		&cli.StringFlag{
			Name:    spec.FlagDevRoot,
			Aliases: []string{spec.FlagNvidiaDevRoot},
		...
		&cli.StringFlag{
			Name:    spec.FlagDeviceIDStrategy,
		...
		&cli.StringFlag{
			Name:    spec.FlagCDIAnnotationPrefix,
		...
		&cli.StringFlag{
			Name:    spec.FlagNvidiaCDIHookPath,
			Aliases: []string{spec.FlagNvidiaCTKPath},
			Value:   spec.DefaultNvidiaCTKPath,
			Usage:   "the path to the nvidia-ctk binary used for NVIDIA CDI hooks in the generated CDI specification",
			EnvVars: []string{"NVIDIA_CDI_HOOK_PATH", "NVIDIA_CTK_PATH"},
		},
		&cli.StringFlag{
			Name:    spec.FlagDriverRootCtrPath,
			Aliases: []string{spec.FlagContainerDriverRoot},
			Value:   spec.DefaultContainerDriverRoot,
			Usage:   "the path where the NVIDIA driver root is mounted in the container for CDI spec generation",
			EnvVars: []string{"DRIVER_ROOT_CTR_PATH", "CONTAINER_DRIVER_ROOT"},
		},
		&cli.StringFlag{
			Name:    spec.FlagMpsRoot,
