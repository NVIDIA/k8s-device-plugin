...
// (no SharedDevicesAllocationPolicy set, or explicitly "distributed") produces
// the same distributed behavior as the existing implementation.
...
func TestFullGPUNodeIgnoresAllocationPolicy(t *testing.T) {
	...
	// logic at the condition level: the aligned path is taken before
	// sharedDevicesAllocationPolicy is ever checked.
	t.Run("AlignedAllocation is selected regardless of sharedDevicesAllocationPolicy", func(t *testing.T) {
...
// pickedFrom were not seeded from required.
