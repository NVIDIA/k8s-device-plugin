...
The `helm` chart for the latest release of the plugin (`v0.7.1`) includes
...
* https://github.com/NVIDIA/k8s-device-plugin/blob/v0.7.1/deployments/helm/nvidia-device-plugin/values.yaml
...
$ helm install \
    --generate-name \
    https://nvidia.github.com/k8s-device-plugin/stable/nvidia-device-plugin-0.7.1.tgz
```
...
It assumes you are pinning to the latest release tag (i.e. `v0.7.1`), but can
...
```shell
$ docker pull nvidia/k8s-device-plugin:v0.7.1
$ docker tag nvidia/k8s-device-plugin:v0.7.1 nvidia/k8s-device-plugin:devel
```
...
$ docker build \
    ...
    https://github.com/NVIDIA/k8s-device-plugin.git#v0.7.1
```
