...
```shell
$ kubectl create -f https://raw.githubusercontent.com/NVIDIA/k8s-device-plugin/v0.8.1/nvidia-device-plugin.yml
```
...
The `helm` chart for the latest release of the plugin (`v0.8.1`) includes
...
* https://github.com/NVIDIA/k8s-device-plugin/blob/v0.8.1/deployments/helm/nvidia-device-plugin/values.yaml
...
$ helm install \
    --generate-name \
    https://nvidia.github.com/k8s-device-plugin/stable/nvidia-device-plugin-0.8.1.tgz
```
...
It assumes you are pinning to the latest release tag (i.e. `v0.8.1`), but can
