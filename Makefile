...
push:
	$(DOCKER) push "$(REGISTRY)/k8s-device-plugin:$(VERSION)-ubuntu18.04"
	$(DOCKER) push "$(REGISTRY)/k8s-device-plugin:$(VERSION)-centos7"
	$(DOCKER) push "$(REGISTRY)/k8s-device-plugin:$(VERSION)-ubi8"

push-short:
	$(DOCKER) tag "$(REGISTRY)/k8s-device-plugin:$(VERSION)-ubuntu18.04" "$(REGISTRY)/k8s-device-plugin:$(VERSION)"
	$(DOCKER) push "$(REGISTRY)/k8s-device-plugin:$(VERSION)"

push-latest:
	$(DOCKER) tag "$(REGISTRY)/k8s-device-plugin:$(VERSION)-ubuntu18.04" "$(REGISTRY)/k8s-device-plugin:latest"
