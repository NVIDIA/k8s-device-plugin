...
func NewConfig(c *cli.Context, flags []cli.Flag) (*Config, error) {
	...
	if c.IsSet("imex-default-channel-strategy") {
		defaultChannelStrategy, err := asDefaultChannelStrategy(c.String("imex-default-channel-strategy"))
		if err != nil {
			return nil, err
		}
		config.Imex.DefaultChannelStrategy = defaultChannelStrategy
	}
